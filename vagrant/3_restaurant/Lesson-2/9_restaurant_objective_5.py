    ...
    def do_GET(self):
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                myResturantQuery = session.query(Restaurant).filter_by(id = restaurantIDPath).one()
                if myResturantQuery != []:
                    self.send_response(200)
                    self.send_header('Content-type', 'text/html')
                    self.end_headers()
                    output = ""
                    output += "<html><body>"
                    output += "<h1>%s</h1>" % myResturantQuery.name
                    output += "<form method='POST' enctype='multipart/form-data' action='/restaurants/%s/edit'>" % restaurantIDPath
                    output += '<input name="newRestaurantName" type="text" placeholder="%s">' % myResturantQuery.name
            ...
            if self.path.endswith("/delete"):
                ...
                if myResturantQuery != []:
                    ...
                    output = ""
                    output += "<html><body>"
                    output += "<h1>Are you sure you want to delete %s?</h1>" % myResturantQuery.name
    ...
    def do_POST(self):
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                # Rename the Restaurant class
                restaurantIDPath = self.path.split("/")[2]
                myResturantQuery = session.query(Restaurant).filter_by(id = restaurantIDPath).one()
                if myResturantQuery != []:
                    myResturantQuery.name = newRestaurantName[0]
                    session.add(myResturantQuery)
                    session.commit()
            ...
            if self.path.endswith("/delete"):
                ...
                myResturantQuery = session.query(Restaurant).filter_by(id = restaurantIDPath).one()
                if myResturantQuery != []:
                    session.delete(myResturantQuery)
                    session.commit()
